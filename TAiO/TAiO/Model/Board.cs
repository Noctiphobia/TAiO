...
namespace TAiO.Model
{
	...
	class Board
	{
		...
		public bool AddBlock(BlockInstance block)
		{
			...
			while (block.LeftUpperCornerY + h > Height)
				Resize();
			BlocksNumber++;
			for (int i = 0; i < h; i++)
				for (int j = 0; j < w; j++)
				{
					if ((Content[i + block.LeftUpperCornerX, j + block.LeftUpperCornerY] & block.Block.Shape[block.BlockVersion][i, j]) > 0)
						return false;
					Content[i + block.LeftUpperCornerX, j + block.LeftUpperCornerY] = block.Block.Shape[block.BlockVersion][i, j] * BlocksNumber;
				}
			return true;
		}
	}
